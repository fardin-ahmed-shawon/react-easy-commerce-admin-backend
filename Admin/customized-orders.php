...
if ($search_query) {
  $where .= " AND (co.order_no LIKE '%$search_query%' OR co.user_phone LIKE '%$search_query%')";
}

if ($status_filter) {
  $where .= " AND co.order_status = '$status_filter'";
}

if ($from_date) {
  $where .= " AND DATE(co.order_date) >= '$from_date'";
}

if ($to_date) {
  $where .= " AND DATE(co.order_date) <= '$to_date'";
}

// Get stats
$stats = array();
foreach (['Pending', 'Processing', 'Completed', 'Cancelled'] as $status) {
  $sql = "SELECT COUNT(id) as count FROM customized_orders WHERE order_status = '$status'";
...
LEFT JOIN customized_products cp ON co.product_id = cp.id
        ...
        <tbody>
          ...
          if (count($ordersData) > 0) {
            foreach ($ordersData as $row) {
              ...
              echo '<td><strong>৳' . number_format($row['advance_amount']) . '</strong></td>';
            ...
            echo '<tr><td colspan="10" class="text-center text-muted py-4">No orders found</td></tr>';
          }
          ?>
        </tbody>
      </table>
