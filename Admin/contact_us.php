...
$page_title = 'Contact Us'; // Set the page title
?>
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submit'])) {
    ...
    if (!empty($description)) {
        ...
        if (mysqli_query($conn, $query)) {
            $product_added_status = "Contact Us content saved successfully!";
        } else {
            $product_added_status = "Error: " . mysqli_error($conn);
        }
    } else {
        $product_added_status = "Please fill in the Contact Us content.";
    }
}
...
<div class="content-wrapper">
            ...
            <div class="row">
              <div class="form-container">
                <h1 class="text-center">Write Contact Us</h1>
                <div class="content">
                    <!-- Product Add form -->
                    <form action="" method="post" enctype="multipart/form-data">
                      <div class="user-details full-input-box">
                        ...
                        <?php
                            $query = "SELECT contact_us FROM footer_info LIMIT 1"; // Adjust LIMIT as needed
                            $result = mysqli_query($conn, $query);

                            if ($result && mysqli_num_rows($result) > 0) {
                                $row = mysqli_fetch_assoc($result);
                                $contact_us_content = $row['contact_us'];
                            } else {
                                $contact_us_content = "";
                            }
                        ?>
                        ...
                        <div class="form-group m-auto"> 
                          <span class="details">Write Contact Us *</span>
                          <textarea id="summernote" rows="4" name="description" cols="58" class="mytextarea">
                            <?php 
                                echo htmlspecialchars($contact_us_content);
                            ?>
                          </textarea>
                        </div>
                        <br><br>
